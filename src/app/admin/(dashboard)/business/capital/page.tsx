...
export default function CapitalPage() {
 ...
 action: { label: 'Hoàn tác', onClick: () => businessService.updateCapital(editingCapital.id, editingCapital).then(fetchData) }
 });
 } else {
 ...
 const { id, ...rest } = item;
 await businessService.createCapital({ ...rest, owner: `${rest.owner} (Bản sao)` });
 ...
 >
 + {t.addCapital}
 </Button> </div> <div className="animate-in fade-in slide-in-from-bottom-2"> <DataTable<Capital>
 data={data}
 isLoading={isLoading}
 columns={[
 ...
 { accessor: 'date', header: t.date, render: (_, c) => new Date(c.date).toLocaleDateString('vi-VN') }
 ]}
 selectable
