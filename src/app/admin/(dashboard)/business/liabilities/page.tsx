...
export default function LiabilitiesPage() {
 ...
 action: { label: 'Hoàn tác', onClick: () => businessService.updateLiability(editingLiability.id, editingLiability).then(fetchData) }
 });
 } else {
 ...
 const { id, ...rest } = item;
 await businessService.createLiability({ ...rest, name: `${rest.name} (Bản sao)` });
 ...
 >
 + {t.addLiability}
 </Button> </div> <div className="animate-in fade-in slide-in-from-bottom-2"> <DataTable<Liability>
 data={data}
 isLoading={isLoading}
 columns={[
 { accessor: 'name', header: t.name },
 { accessor: 'type', header: t.type },
 { accessor: 'dueDate', header: t.dueDate, render: (_, l) => new Date(l.dueDate).toLocaleDateString('vi-VN') },
