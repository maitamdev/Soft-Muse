...
export default function PurchaseOrdersPage() {
 ...
 const { id, ...rest } = item;
 await businessService.createPurchaseOrder({ ...rest, 
 ...
 return (
 <div className="space-y-6" dir="ltr"> <div className="flex items-center justify-between"> <div> <h1 className="text-2xl font-bold text-[var(--admin-text-base)]">{t.title}</h1> <p className="text-sm text-[var(--admin-text-muted)] mt-1">{t.subtitle}</p> </div> <Button variant="primary" onClick={() => router.push('/admin/business/purchase-orders/new')}>
 + {t.addPO}
 </Button> </div> <div className="animate-in fade-in slide-in-from-bottom-2"> <DataTable<PurchaseOrder>
 data={data}
 isLoading={isLoading}
 columns={[
 { accessor: 'reference', header: t.poNumber },
 { accessor: 'date', header: adminAr.dashboard.date, render: (_, p) => new Date(p.date).toLocaleDateString('vi-VN') },
 ...
 title="Xóa "
 description="Bạn có chắc muốn Xóa vĩnh viễnً؟ Không thể hoàn tác thao tác này Xóa Tất cả."
