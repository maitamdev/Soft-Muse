...
export default function InventoryPage() {
 ...
 > <div className="space-y-4 py-1"> <div> <p className="text-sm text-[var(--admin-text-muted)] mb-1">Tồn kho hiện tại</p> <p className="text-2xl font-bold text-[var(--admin-text-base)] tabular-nums">{adjustModal.product?.stock ?? 0}</p> </div>{adjustModal.product?.variants.length ? <label className="flex flex-col gap-1.5 text-sm font-medium text-[var(--admin-text-subtle)]">Biến thể<select value={adjustVariantId} onChange={(event) => setAdjustVariantId(event.target.value)} className="h-11 border border-[var(--admin-border-base)] bg-[var(--admin-bg-surface)] px-3 text-sm"><option value="">Chọn biến thể</option>{adjustModal.product.variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.size} · {variant.sku} · tồn {variant.stock}</option>)}</select></label> : null}<div> <p className="text-sm font-medium text-[var(--admin-text-subtle)] mb-1.5">Loại giao dịch</p> <div className="flex gap-1 bg-[var(--admin-bg-elevated)] border border-[var(--admin-border-base)] rounded-[var(--admin-radius-md)] p-1">
