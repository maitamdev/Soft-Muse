...
export default function StockMovementsPage() {
  ...
  return <div className="space-y-6 pb-12">
    ...
    <Modal isOpen={adjustOpen} onClose={() => setAdjustOpen(false)} title="Điều chỉnh tồn kho" size="sm" footer={<div className="flex justify-end gap-3"><Button variant="ghost" onClick={() => setAdjustOpen(false)}>Hủy</Button><Button isLoading={saving} onClick={() => void submitAdjustment()}>Lưu điều chỉnh</Button></div>}>
      ...
      {selectedProduct?.variants.length ? <label className="flex flex-col gap-1 text-sm font-medium">Biến thể<select value={variantId} onChange={(event) => setVariantId(event.target.value)} className="h-11 border border-[var(--admin-border-base)] bg-[var(--admin-bg-surface)] px-3"><option value="">Chọn biến thể</option>{selectedProduct.variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.size} · tồn {variant.stock}</option>)}</select></label> : null}
