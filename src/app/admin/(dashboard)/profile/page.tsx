...
export default function ProfilePage() {
 ...
 toast.error('Mật khẩu mới không');
 return;
 }
 setSaving(true);
 try {
 ...
 toast.success('đãKết thúc phiên thành công');
 ...
 {activeTab === 'personal' && (
 <form onSubmit={handleSavePersonal} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-2xl"> <div className="border-b border-[var(--admin-border-light)] pb-4"> <h3 className="text-xl font-bold text-[var(--admin-text-base)]">{adminAr.profile.tabs.personalInfo}</h3> <p className="text-sm text-[var(--admin-text-muted)] mt-1">Thông tin cá nhân.</p> </div> <AvatarUpload 
 ...
 /> <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> <div> <label className="block text-sm font-medium mb-1.5 text-[var(--admin-text-base)]">{adminAr.profile.personal.name}</label> <Input type="text" value={profile.name} onChange={(e) => setProfile({ ...profile, name: e.target.value})} /> </div> <div> <label className="block text-sm font-medium mb-1.5 text-[var(--admin-text-base)]">mã </label> <Input type="text" dir="ltr" value={profile.username} onChange={(e) => setProfile({ ...profile, username: e.target.value})} /> </div> <div> <label className="block text-sm font-medium mb-1.5 text-[var(--admin-text-base)]">{adminAr.profile.personal.email}</label> <Input type="email" value={profile.email} onChange={(e) => setProfile({ ...profile, email: e.target.value})} /> </div> <div> <label className="block text-sm font-medium mb-1.5 text-[var(--admin-text-base)]">{adminAr.profile.personal.phone}</label> <Input type="text" value={profile.phone} onChange={(e) => setProfile({ ...profile, phone: e.target.value})} /> </div> <div> <label className="block text-sm font-medium mb-1.5 text-[var(--admin-text-base)]">{adminAr.profile.personal.role}</label> <Input type="text" value={profile.role} disabled className="bg-[var(--admin-bg-subtle)] text-[var(--admin-text-muted)] cursor-not-allowed opacity-70" /> </div> <div className="md:col-span-2"> <label className="block text-sm font-medium mb-1.5 text-[var(--admin-text-base)]">{adminAr.profile.personal.bio}</label> <textarea rows={3} value={profile.bio} onChange={(e) => setProfile({ ...profile, bio: e.target.value})} className="w-full px-4 py-2 bg-[var(--admin-bg-base)] border border-[var(--admin-border-base)] rounded-[var(--admin-radius-md)] outline-none focus:border-[var(--admin-primary)] focus:ring-1 focus:ring-[var(--admin-primary)] text-sm resize-y" /> </div> </div>

 {/* Read-only: role, account facts & effective permissions */}
 <div className="rounded-[var(--admin-radius-lg)] border border-[var(--admin-border-base)] bg-[var(--admin-bg-elevated)] p-5 space-y-4"> <div className="grid grid-cols-1 sm:grid-cols-3 gap-4"> <div> <p className="text-xs text-[var(--admin-text-muted)] mb-1">Vai trò</p> <p className="text-sm font-bold text-[var(--admin-text-base)]">{roleRecord?.nameAr ?? profile.role}</p> </div> <div> <p className="text-xs text-[var(--admin-text-muted)] mb-1">Lần đăng nhập cuối</p> <p className="text-sm font-semibold text-[var(--admin-text-base)] tabular-nums">{staffRecord?.lastLoginAt ? formatDate(staffRecord.lastLoginAt) : '—'}</p> </div> <div> <p className="text-xs text-[var(--admin-text-muted)] mb-1">ngày</p> <p className="text-sm font-semibold text-[var(--admin-text-base)] tabular-nums">{staffRecord?.createdAt ? formatDate(staffRecord.createdAt) : '—'}</p> </div> </div>

 {roleRecord && (
 <div> <p className="text-xs font-semibold text-[var(--admin-text-subtle)] uppercase tracking-wider mb-2">Quyền hạn </p> <div className="flex flex-wrap gap-1.5">
 {Object.keys(PERMISSION_MODULES_AR).map((mod) => {
 const p = roleRecord.permissions[mod] ?? { read: false, write: false, delete: false };
 const level = p.delete ? '' : p.write ? 'Sửa' : p.read ? 'Đọc' : '—';
 ...
 </div> <div className="flex justify-end pt-6 border-t border-[var(--admin-border-light)]"> <Button type="submit" isLoading={saving} leftIcon={<IconDeviceFloppy size={18} />}>
 ...
 className="h-10 px-3 w-full bg-[var(--admin-bg-base)] border border-[var(--admin-border-base)] rounded-[var(--admin-radius-md)] text-sm outline-none focus:ring-2 focus:ring-[var(--admin-primary)]"
 > <option value="light"></option> <option value="dark"></option> <option value="system"></option> </select> </div> </div> <div className="space-y-3"> <label className="flex items-center justify-between p-4 border border-[var(--admin-border-base)] rounded-[var(--admin-radius-lg)] bg-[var(--admin-bg-elevated)] cursor-pointer"> <span className="text-sm font-medium text-[var(--admin-text-base)]">Email</span> <input
 type="checkbox"
 ...
 /> </label> <label className="flex items-center justify-between p-4 border border-[var(--admin-border-base)] rounded-[var(--admin-radius-lg)] bg-[var(--admin-bg-elevated)] cursor-pointer"> <span className="text-sm font-medium text-[var(--admin-text-base)]">Thông báo </span> <input
 type="checkbox"
 checked={profile.preferences.pushNotifications}
 onChange={(e) => setProfile({ ...profile, preferences: { ...profile.preferences, pushNotifications: e.target.checked } })}
 className="w-5 h-5 accent-[var(--admin-primary)]"
 /> </label> </div> <div className="flex justify-end pt-6 border-t border-[var(--admin-border-light)]"> <Button type="submit" isLoading={saving} leftIcon={<IconDeviceFloppy size={18} />}>
 {saving ? adminAr.table.loading : adminAr.common.save}
 </Button> </div> </form>
 )}

 {activeTab === 'sessions' && (
 <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-2xl"> <div className="border-b border-[var(--admin-border-light)] pb-4"> <h3 className="text-xl font-bold text-[var(--admin-text-base)]">{adminAr.profile.tabs.sessions}</h3> <p className="text-sm text-[var(--admin-text-muted)] mt-1">nàyTất cả Đăng nhập. không.</p> </div> <div className="space-y-4">
 {profile.sessions.map((session) => (
 <div key={session.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-5 border border-[var(--admin-border-base)] rounded-[var(--admin-radius-lg)] bg-[var(--admin-bg-elevated)] gap-4 transition-colors hover:border-[var(--admin-border-strong)]"> <div className="flex items-center gap-4"> <div className="w-12 h-12 rounded-[var(--admin-radius-md)] bg-[var(--admin-bg-base)] flex items-center justify-center text-[var(--admin-text-subtle)] border border-[var(--admin-border-light)] shadow-sm shrink-0">
 ...
 </div> <div> <h4 className="font-semibold text-[var(--admin-text-base)] flex items-center gap-2 mb-0.5">
 {session.device}
 {session.isCurrent && <Badge variant="success" className="px-1.5 py-0"></Badge>}
 </h4> <div className="text-xs text-[var(--admin-text-muted)] flex items-center gap-2 font-medium"> <span>{session.ip}</span> <span className="text-[var(--admin-border-strong)]">&bull;</span> <span>{session.location}</span> </div> <p className="text-xs text-[var(--admin-text-subtle)] mt-1.5 tabular-nums">{adminAr.profile.sessions.lastActive}: {formatDate(session.lastActive)}</p> </div> </div>
