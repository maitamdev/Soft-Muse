...
function Avatar({ name, superAdmin }: { name: string; superAdmin: boolean }) {
  return (
    <div className="relative shrink-0">
      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white ${superAdmin ? 'bg-[var(--admin-primary)]' : 'bg-[var(--admin-bg-elevated)] text-[var(--admin-text-base)] border border-[var(--admin-border-base)]'}`}>
        {name.charAt(0)}
      </div>
      {superAdmin && (
        <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-yellow-400 rounded-full border-2 border-[var(--admin-bg-surface)] flex items-center justify-center">
          <IconShieldCheck size={9} className="text-yellow-900" />
        </div>
      )}
    </div>
  );
...
export default function UsersPage() {
  ...
  return (
    <StaggerContainer className="space-y-6 pb-12">
          ...
          actions={
            <div className="flex items-center gap-3">
              <Link href="/admin/users/roles">
                <Button variant="secondary" size="md" leftIcon={<IconShieldCheck size={18} />}>
                  {adminAr.sidebar.roles}
                </Button>
              </Link>
              <Button variant="primary" size="md" leftIcon={<IconUserPlus size={18} />} onClick={openCreate}>
                {t.addUser}
              </Button>
            </div>
          }
      ...
      {/* KPI Cards */}
      <StaggerItem className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <KpiCard title="إجمالي الموظفين" value={staff.length} icon={<IconUser stroke={2} />} accentColor="blue" />
        <KpiCard title="نشطون" value={activeCount} icon={<IconUser stroke={2} />} accentColor="emerald" />
        <KpiCard title="معلقون" value={inactiveCount} icon={<IconUser stroke={2} />} accentColor="orange" />
        <KpiCard title="مسؤولون أوائل" value={superAdmins} icon={<IconShieldCheck stroke={2} />} accentColor="purple" />
      </StaggerItem>
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="divide-y divide-[var(--admin-border-light)]">
                {staff.map(member => (
                  <div key={member.id} className="flex items-center gap-4 px-6 py-4 hover:bg-[var(--admin-bg-hover)] transition-colors group">
                    <Avatar name={member.nameAr} superAdmin={member.isSuperAdmin} />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-bold text-[var(--admin-text-base)] text-sm">{member.nameAr}</p>
                        {member.isSuperAdmin && (
      ...
      >
        <div className="space-y-4 py-1">
          ...
          />
          <Input
            label="رابط الصورة الرمزية (اختياري)"
            placeholder="https://..."
            dir="ltr"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Input
              label={modal.editing ? 'كلمة مرور جديدة (اختياري)' : 'كلمة المرور'}
              type="password"
          ...
          <div>
            <label className="block text-sm font-semibold text-[var(--admin-text-base)] mb-1.5">{t.role}</label>
            <select
              value={form.roleId}
          ...
          <div>
            <label className="block text-sm font-semibold text-[var(--admin-text-base)] mb-1.5">{t.status}</label>
      ...
      >
        <p className="text-sm text-[var(--admin-text-muted)] py-2">
          ...
          <br />
          <span className="text-[var(--admin-danger)]">{adminAr.common.thisActionCannotBeUndone}</span>
        </p>
      </Modal>
    </StaggerContainer>
  );
