...
export default function RolesPage() {
 ...
 if (!form.nameAr.trim()) { toast.error('Tênvai trò'); return; }
 setSaving(true);
 try {
 if (modal.editing) {
 ...
 return (
 <StaggerContainer className="space-y-6 pb-12"> <StaggerItem> <div className="flex items-center gap-2 text-sm text-[var(--admin-text-muted)] mb-2"> <Link href="/admin/users" className="hover:text-[var(--admin-primary)] transition-colors">Người dùng</Link> <IconChevronRight size={14} /> <span className="text-[var(--admin-text-base)] font-medium">{adminAr.sidebar.roles}</span> </div> <PageHeader
 title={t.title}
 ...
 className={cn(
 "w-full text-left p-4 rounded-[var(--admin-radius-xl)] border transition-all",
 ...
 > <div className="flex items-start justify-between gap-3"> <div className="flex items-center gap-3 min-w-0"> <span className={cn("w-3 h-3 rounded-full shrink-0", `bg-${role.color}-500`)} /> <div className="min-w-0"> <p className="font-bold text-sm text-[var(--admin-text-base)] truncate">{role.nameAr}</p> <p className="text-xs text-[var(--admin-text-muted)] truncate mt-0.5">{role.descriptionAr}</p> </div> </div> <div className="flex flex-col items-end gap-1.5 shrink-0">
 {role.isSystem && (
 <div className="flex items-center gap-1 text-xs text-[var(--admin-text-subtle)]"> <IconLock size={10} /> <span>{t.systemRole}</span> </div>
 )}
 <span className="text-xs font-semibold text-[var(--admin-text-muted)]">{role.staffCount} </span> </div> </div> </button>
 ))}
 </div>
 ...
 </div> </CardHeader> <CardContent className="p-0"> <div className="overflow-x-auto"> <table className="w-full text-sm"> <thead> <tr className="border-b border-[var(--admin-border-light)] bg-[var(--admin-bg-elevated)]"> <th className="text-left px-4 py-3 text-[var(--admin-text-subtle)] font-semibold text-xs uppercase tracking-wider">{t.module}</th> <th className="text-center px-4 py-3 text-[var(--admin-text-subtle)] font-semibold text-xs uppercase tracking-wider w-20">{t.read}</th> <th className="text-center px-4 py-3 text-[var(--admin-text-subtle)] font-semibold text-xs uppercase tracking-wider w-20">{t.write}</th> <th className="text-center px-4 py-3 text-[var(--admin-text-subtle)] font-semibold text-xs uppercase tracking-wider w-20">{t.delete}</th> </tr> </thead> <tbody className="divide-y divide-[var(--admin-border-light)]">
 {PERM_KEYS.map(module => {
 const p = displayRole.permissions[module] ?? { read: false, write: false, delete: false };
 return (
 <tr key={module} className="hover:bg-[var(--admin-bg-hover)] transition-colors"> <td className="px-4 py-3 font-medium text-[var(--admin-text-base)]">
 {PERMISSION_MODULES_AR[module]}
 </td> <td className="px-4 py-3 text-center"><PermissionCell value={p.read} /></td> <td className="px-4 py-3 text-center"><PermissionCell value={p.write} /></td> <td className="px-4 py-3 text-center"><PermissionCell value={p.delete} /></td> </tr>
 );
 })}
 </tbody> </table> </div> </CardContent> </Card>
 ) : (
 <div className="h-full flex items-center justify-center text-[var(--admin-text-muted)] p-12"> <div className="text-center"> <IconShieldCheck size={48} className="mx-auto mb-3 opacity-30" /> <p className="font-medium"></p> </div> </div>
 )}
 </div> </StaggerItem>
 ...
 > <div className="space-y-5 py-1"> <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> <Input
 label={t.roleName}
 placeholder=":Sản phẩm"
 value={form.nameAr}
 onChange={e => setForm(f => ({ ...f, nameAr: e.target.value }))}
 /> <div> <label className="block text-sm font-semibold text-[var(--admin-text-base)] mb-1.5"></label> <div className="flex flex-wrap gap-2">
 {COLORS.map(c => (
 <button
 key={c}
 type="button"
 ...
 <div> <p className="text-sm font-semibold text-[var(--admin-text-base)] mb-3">{t.permissions}</p> <div className="overflow-x-auto rounded-[var(--admin-radius-xl)] border border-[var(--admin-border-base)]"> <table className="w-full text-sm"> <thead> <tr className="bg-[var(--admin-bg-elevated)] border-b border-[var(--admin-border-light)]"> <th className="text-left px-4 py-2.5 text-xs font-semibold text-[var(--admin-text-subtle)] uppercase tracking-wider">{t.module}</th> <th className="text-center px-4 py-2.5 text-xs font-semibold text-[var(--admin-text-subtle)] uppercase tracking-wider w-20">{t.read}</th> <th className="text-center px-4 py-2.5 text-xs font-semibold text-[var(--admin-text-subtle)] uppercase tracking-wider w-20">{t.write}</th> <th className="text-center px-4 py-2.5 text-xs font-semibold text-[var(--admin-text-subtle)] uppercase tracking-wider w-20">{t.delete}</th> </tr> </thead> <tbody className="divide-y divide-[var(--admin-border-light)]">
 {PERM_KEYS.map(module => {
 const p = form.permissions[module] ?? { read: false, write: false, delete: false };
 return (
 <tr key={module} className="hover:bg-[var(--admin-bg-hover)] transition-colors"> <td className="px-4 py-2.5 font-medium text-[var(--admin-text-base)] text-sm">
 {PERMISSION_MODULES_AR[module]}
 </td>
 {(['read', 'write', 'delete'] as const).map(key => (
 <td key={key} className="px-4 py-2.5 text-center"> <button
 type="button"
