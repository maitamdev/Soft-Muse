...
export default function BannerManager() {
 ...
 <div className="w-full md:w-[450px] shrink-0 border-r border-[var(--admin-border-base)] flex flex-col h-full bg-[var(--admin-bg-surface)]"> <div className="p-4 border-b border-[var(--admin-border-light)] flex items-center justify-between"> <div className="flex items-center gap-2 text-[var(--admin-text-base)]"> <IconSlideshow size={20} className="text-[var(--admin-primary)]" /> <h1 className="font-bold">Banner Manager</h1> </div> <Button variant="primary" size="sm" leftIcon={<IconPlus size={16} />}>
 Add Banner
 </Button> </div> <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-4"> <StaggerContainer>
 {banners.map(banner => (
 <StaggerItem key={banner.id}> <div
 ...
 > <div className="flex justify-between items-start mb-3"> <div> <Badge variant={banner.status === 'active' ? 'success' : banner.status === 'scheduled' ? 'warning' : 'neutral'} size="sm" className="mb-2 uppercase">
 {banner.status}
 </Badge> <h3 className="font-bold text-[var(--admin-text-base)] capitalize">{banner.type} Banner</h3> <p className="text-xs font-medium text-[var(--admin-text-muted)] mt-1">Priority: {banner.priority}</p> </div> <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"> <button onClick={(e) => { e.stopPropagation(); toggleStatus(banner.id, banner.status); }} className="p-1 text-[var(--admin-text-subtle)] hover:text-[var(--admin-text-base)]">
 {banner.status === 'active' ? <IconEyeOff size={16} /> : <IconEye size={16} />}
 </button> <button className="p-1 text-[var(--admin-text-subtle)] hover:text-[var(--admin-primary)]"> <IconEdit size={16} /> </button> <button onClick={(e) => { e.stopPropagation(); deleteBanner(banner.id); }} className="p-1 text-[var(--admin-danger-muted)] hover:text-[var(--admin-danger)]"> <IconTrash size={16} /> </button> </div> </div>

 {banner.mediaUrl && (
 <div className="w-full h-24 rounded-lg overflow-hidden bg-slate-100 relative">
 { }
 <img src={banner.mediaUrl} className="absolute inset-0 w-full h-full object-cover" alt="Banner preview" /> </div>
 )}
 </div> </StaggerItem>
 ))}
 </StaggerContainer> </div> </div>

 {/* Right Panel: Live Preview */}
 <div className="flex-1 min-w-0 h-full p-4 pl-0"> <CMSPreviewPanel
 view={view}
 ...
 <div className="absolute inset-0 bg-black transition-opacity" style={{ opacity: activeBanner.overlayOpacity / 100 }} /> <div className="relative z-10 p-4 text-center">
