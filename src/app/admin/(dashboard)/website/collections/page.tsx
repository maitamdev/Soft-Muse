  ...
  return (
    <FadeIn className="space-y-5 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-[var(--admin-text-base)]">{t.title}</h2>
          <p className="text-sm font-medium text-[var(--admin-text-muted)] mt-1">{t.subtitle}</p>
        </div>
        <Link
      ...
      <div className="flex items-start gap-2 text-xs font-medium text-[var(--admin-text-muted)] bg-[var(--admin-bg-elevated)] border border-[var(--admin-border-light)] rounded-[var(--admin-radius-lg)] px-3 py-2.5">
        <IconInfoCircle size={16} className="shrink-0 mt-px text-[var(--admin-primary)]" />
        <span>{t.dragToReorder} — {t.featuredHint}</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-10 text-center text-[var(--admin-text-muted)] font-medium border border-dashed border-[var(--admin-border-base)] rounded-[var(--admin-radius-xl)]">
          <IconStack2 size={36} className="mx-auto mb-3 text-[var(--admin-text-subtle)]" />
          {t.empty}
        </div>
      ) : (
        <SortableList
          ...
          renderItem={(c) => (
            <SortableItem id={c.id} key={c.id}>
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-[var(--admin-radius-md)] overflow-hidden bg-[var(--admin-bg-elevated)] shrink-0 flex items-center justify-center">
                  {c.image ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-bold text-[var(--admin-text-base)] truncate">{c.name}</p>
                    {c.display.featured && (
                      <Badge variant="warning" size="sm">{t.featured}</Badge>
                    )}
                  </div>
                  <p className="text-xs font-medium text-[var(--admin-text-subtle)] mt-0.5">
                    {c.display.visible ? t.visible : t.hidden} · {c.productIds.length} منتج
                  </p>
                </div>
