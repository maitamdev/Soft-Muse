...
.finally(() => setLoading(false));
 ...
 if (!settings) return;
 const newLink = { id: `l-${Date.now()}`, label: 'Mới', url: '/' };
 setSettings({ ...settings,
 ...
 {/* ── Left panel: settings ── */}
 <div className="w-full md:w-[420px] shrink-0 border-r border-[var(--admin-border-base)] flex flex-col h-full bg-[var(--admin-bg-surface)]"> <div className="p-4 border-b border-[var(--admin-border-light)] flex items-center justify-between sticky top-0 bg-[var(--admin-bg-surface)] z-10"> <div className="flex items-center gap-2 text-[var(--admin-text-base)]"> <IconLayoutBottombar size={20} className="text-[var(--admin-primary)]" /> <h1 className="font-bold">Chân trang (Footer)</h1> </div> <Button
 variant="primary"
 size="sm"
 ...
 >
 Lưu
 </Button> </div> <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-6">

 {/* Newsletter */}
 <section className="space-y-3"> <h3 className="text-xs font-bold text-[var(--admin-text-muted)] uppercase">Bản tin</h3> <label className="flex items-center gap-3 cursor-pointer"> <input
 type="checkbox"
 ...
 <> <div> <label className="block text-xs font-bold text-[var(--admin-text-muted)] mb-1">Địa chỉ</label> <input type="text" value={settings.newsletterTitle} onChange={(e) => set('newsletterTitle', e.target.value)} className={inputCls} /> </div> <div> <label className="block text-xs font-bold text-[var(--admin-text-muted)] mb-1"></label> <input type="text" value={settings.newsletterSubtitle} onChange={(e) => set('newsletterSubtitle', e.target.value)} className={inputCls} /> </div> </>
 )}
 </section>

 {/* Social / Payment */}
 <section className="space-y-3"> <h3 className="text-xs font-bold text-[var(--admin-text-muted)] uppercase"></h3> <label className="flex items-center gap-3 cursor-pointer"> <input
 type="checkbox"
 checked={settings.showSocialIcons}
 onChange={(e) => set('showSocialIcons', e.target.checked)}
 className="w-4 h-4 rounded text-[var(--admin-primary)]"
 /> <span className="text-sm font-bold text-[var(--admin-text-base)]"></span> </label> <label className="flex items-center gap-3 cursor-pointer"> <input
 type="checkbox"
 checked={settings.showPaymentIcons}
 onChange={(e) => set('showPaymentIcons', e.target.checked)}
 className="w-4 h-4 rounded text-[var(--admin-primary)]"
 /> <span className="text-sm font-bold text-[var(--admin-text-base)]"></span> </label> </section>

 {/* Copyright */}
 <section className="space-y-3"> <h3 className="text-xs font-bold text-[var(--admin-text-muted)] uppercase"></h3> <div> <label className="block text-xs font-bold text-[var(--admin-text-muted)] mb-1"></label> <input type="text" value={settings.copyrightText} onChange={(e) => set('copyrightText', e.target.value)} className={inputCls} /> </div> <div> <label className="block text-xs font-bold text-[var(--admin-text-muted)] mb-1"></label> <input type="text" value={settings.developerCredit} onChange={(e) => set('developerCredit', e.target.value)} className={inputCls} /> </div> </section>

 {/* Navigation Columns */}
 <section className="space-y-3"> <div className="flex items-center justify-between"> <h3 className="text-xs font-bold text-[var(--admin-text-muted)] uppercase"></h3> <button
 onClick={addColumn}
 ...
 <p className="text-sm text-[var(--admin-text-muted)] text-center py-4">Không có.</p>
 )}
...
.map((col) => (
 ...
 {/* Column header */}
 <div className="flex items-center gap-2 p-3 bg-[var(--admin-bg-elevated)]"> <button
 ...
 {col.links.map((link) => (
 <div key={link.id} className="flex items-center gap-2"> <div className="flex-1 grid grid-cols-[1fr_1fr] gap-1"> <input
 type="text"
 value={link.label}
 ...
 <div className="flex-1 min-w-0 h-full p-4 pl-0"> <CMSPreviewPanel view={view} onViewChange={setView} title="AURA" isUpdating={saving}> <div className="w-full h-full flex flex-col font-sans bg-[#FAF8F5]" dir="ltr"> <div className="flex-1 flex items-center justify-center opacity-20 text-sm text-gray-400 font-bold">
 trang</div> <footer className="border-t border-[#EAE3D9] bg-[#FAF8F5]">
 {/* Newsletter strip */}
 {settings.showNewsletter && (
 <div className="border-b border-[#EAE3D9] py-8 px-8 text-left"> <p className="text-[10px] uppercase tracking-widest text-[#C5A880] font-bold mb-1">{settings.newsletterTitle}</p> <h3 className="text-lg font-light text-[#1C1C1B] mb-4">{settings.newsletterSubtitle}</h3> <div className="flex max-w-xs gap-0"> <input type="email" placeholder="Email của bạn." className="flex-1 h-10 border border-[#EAE3D9] border-l-0 px-4 text-xs bg-white outline-none" /> <button className="h-10 bg-[#1C1C1B] text-[#FAF8F5] px-5 text-xs font-semibold">Thao tác</button> </div> </div>
 )}
 ...
 <div key={col.id}> <p className="text-[10px] uppercase tracking-widest text-[#C5A880] font-bold mb-3">{col.title}</p> <ul className="space-y-2">
 {col.links.map((link) => (
 ...
 {/* Bottom bar */}
 <div className="border-t border-[#EAE3D9] py-4 px-8 flex items-center justify-between"> <div className="text-[10px] text-[#8A8070]">{settings.copyrightText}</div>
 ...
 <div className="text-[10px] text-[#C5A880]">{settings.developerCredit}</div> </div> </footer> </div> </CMSPreviewPanel> </div> </FadeIn>
 );
}
