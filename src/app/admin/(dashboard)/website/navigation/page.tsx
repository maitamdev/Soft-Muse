...
export default function NavigationManager() {
  ...
  async function loadMenus() {
    try {
      ...
      toast.error("فشل في تحميل قوائم التنقل");
    } finally {
      setLoading(false);
    }
  }
  ...
  const LOCATION_LABELS: Record<string, string> = {
    header: 'الهيدر (Header)',
    footer: 'الفوتر (Footer)',
    mega_menu: 'القائمة الكبيرة',
  };
  ...
  if (loading) return <div className="p-8 animate-pulse text-[var(--admin-text-muted)]">جاري التحميل...</div>;
  ...
  return (
    <FadeIn className="h-[calc(100vh-180px)] min-h-[560px] flex flex-col md:flex-row bg-[var(--admin-bg-base)] rounded-[var(--admin-radius-xl)] overflow-hidden border border-[var(--admin-border-base)]">
        ...
        {editingItem ? (
          ...
          <div className="flex flex-col h-full">
            <div className="p-4 border-b border-[var(--admin-border-light)] flex items-center gap-2">
              <button onClick={() => setEditingItem(null)} className="p-1.5 rounded-md hover:bg-[var(--admin-bg-hover)] text-[var(--admin-text-muted)]">
                <IconChevronLeft size={18} />
              </button>
              <p className="font-bold text-sm text-[var(--admin-text-base)]">تعديل الرابط</p>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
              <div>
                <label className={labelCls}>النص (Label)</label>
                <input className={inputCls} value={editingItem.label} onChange={e => setEditingItem({ ...editingItem, label: e.target.value })} />
              </div>
              <div>
                <label className={labelCls}>الرابط (URL)</label>
                <input className={inputCls + " dir-ltr text-left"} value={editingItem.url} onChange={e => setEditingItem({ ...editingItem, url: e.target.value })} />
              </div>
              <div>
                <label className={labelCls}>المجموعة</label>
                <select className={inputCls} value={editingItem.group} onChange={e => setEditingItem({ ...editingItem, group: e.target.value as 'primary' | 'secondary' })}>
                  <option value="primary">رئيسية (يسار)</option>
                  <option value="secondary">ثانوية (يمين)</option>
                </select>
              </div>
              <div>
                <label className={labelCls}>شارة (Badge) — اختياري</label>
                <input className={inputCls} value={editingItem.badge || ''} onChange={e => setEditingItem({ ...editingItem, badge: e.target.value })} placeholder="مثال: جديد" />
              </div>
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={editingItem.openInNewTab} onChange={e => setEditingItem({ ...editingItem, openInNewTab: e.target.checked })} className="w-4 h-4 rounded text-[var(--admin-primary)]" />
                <span className="text-sm text-[var(--admin-text-base)]">فتح في تبويب جديد</span>
              </label>
            </div>
            <div className="p-4 border-t border-[var(--admin-border-light)] flex gap-2">
              <Button variant="secondary" size="sm" onClick={() => setEditingItem(null)} className="flex-1">إلغاء</Button>
              <Button variant="primary" size="sm" isLoading={saving} leftIcon={<IconDeviceFloppy size={15} />} onClick={handleSaveEdit} className="flex-1">حفظ</Button>
            </div>
          </div>
        ) : (
          <>
            <div className="p-4 border-b border-[var(--admin-border-light)] flex items-center justify-between">
              <div className="flex items-center gap-2 text-[var(--admin-text-base)]">
                <IconSitemap size={20} className="text-[var(--admin-primary)]" />
                <h1 className="font-bold">إدارة التنقل</h1>
              </div>
              <Button variant="primary" size="sm" leftIcon={<IconPlus size={16} />} onClick={() => setShowAddForm(s => !s)}>
                إضافة رابط
              </Button>
            </div>
            ...
            {/* Add item form */}
            {showAddForm && (
              <div className="p-4 border-b border-[var(--admin-border-light)] bg-[var(--admin-bg-elevated)] space-y-3">
                <p className="text-xs font-bold text-[var(--admin-text-base)]">رابط جديد</p>
                <input className={inputCls} placeholder="النص (Label)" value={newItem.label} onChange={e => setNewItem({ ...newItem, label: e.target.value })} />
                <input className={inputCls + " dir-ltr text-left"} placeholder="/shop" value={newItem.url} onChange={e => setNewItem({ ...newItem, url: e.target.value })} />
                <select className={inputCls} value={newItem.group} onChange={e => setNewItem({ ...newItem, group: e.target.value as 'primary' | 'secondary' })}>
                  <option value="primary">رئيسية (يسار)</option>
                  <option value="secondary">ثانوية (يمين)</option>
                </select>
                <div className="flex gap-2">
                  <Button variant="secondary" size="sm" onClick={() => { setShowAddForm(false); setNewItem(EMPTY_NEW_ITEM); }} className="flex-1">إلغاء</Button>
                  <Button variant="primary" size="sm" isLoading={saving} onClick={handleAddItem} className="flex-1">إضافة</Button>
                </div>
              </div>
            )}
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0 flex-1 cursor-pointer" onClick={() => setEditingItem(item)}>
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <p className="text-sm font-bold text-[var(--admin-text-base)] truncate">{item.label}</p>
                            ...
                            <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${item.group === 'primary' ? 'bg-[var(--admin-primary-muted)] text-[var(--admin-primary)]' : 'bg-[var(--admin-bg-active)] text-[var(--admin-text-muted)]'}`}>
                              {item.group === 'primary' ? 'رئيسي' : 'ثانوي'}
                            </span>
                          </div>
                          <p className="text-xs text-[var(--admin-text-muted)] mt-0.5 dir-ltr text-right">{item.url}</p>
                        </div>
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                          <button onClick={() => setEditingItem(item)} className="p-1.5 text-[var(--admin-text-subtle)] hover:text-[var(--admin-primary)] rounded hover:bg-[var(--admin-bg-hover)]">
                            <IconEdit size={15} />
                          </button>
                          <button onClick={() => handleDeleteItem(item.id)} className="p-1.5 text-[var(--admin-danger-muted)] hover:text-[var(--admin-danger)] rounded hover:bg-[var(--admin-danger)]/10">
                            <IconTrash size={15} />
                          </button>
                        </div>
                      </div>
      ...
      {/* Right Panel: Preview */}
      <div className="flex-1 min-w-0 h-full p-4 ps-0">
        <CMSPreviewPanel view={view} onViewChange={setView} title="معاينة القائمة" isUpdating={saving}>
          <div className="w-full h-full bg-white flex flex-col font-sans" dir="rtl">
            <header className="h-20 border-b flex items-center justify-between px-8 gap-4">
              ...
              <div className="text-2xl font-black tracking-tighter text-gray-900 shrink-0">AURA</div>
              <nav className="flex items-center gap-6">
            ...
            <div className="flex-1 bg-slate-50 flex items-center justify-center text-slate-300 text-sm">
              محتوى الصفحة
            </div>
          </div>
        </CMSPreviewPanel>
