...
export default function NavigationManager() {
 ...
 toast.error("Không thể tải menu website.");
 } finally {
 setLoading(false);
 }
 }
 ...
 toast.error("Không thể lưu menu website.");
 } finally {
 setSaving(false);
 }
 };
 ...
 toast.error("Không thể đổi thứ tự menu.");
 loadMenus();
 }
 };
 ...
 toast.success("Đã xóa liên kết khỏi menu.");
 };

 const handleSaveEdit = async () => {
 if (!activeMenu || !editingItem) return;
 if (!editingItem.label.trim() || !isValidMenuUrl(editingItem.url)) { toast.error("Tên hiển thị hoặc đường dẫn chưa hợp lệ."); return; }
 ...
 if (!activeMenu || !newItem.label.trim() || !isValidMenuUrl(newItem.url)) {
 toast.error("Vui lòng nhập tên và đường dẫn hợp lệ (/duong-dan hoặc https://...).");
 return;
 }
 ...
 toast.success("Đã thêm liên kết vào menu.");
 };
 ...
 {editingItem ? (
 /* Item editor panel */
 <div className="flex flex-col h-full"> <div className="p-4 border-b border-[var(--admin-border-light)] flex items-center gap-2"> <button onClick={() => setEditingItem(null)} className="p-1.5 rounded-md hover:bg-[var(--admin-bg-hover)] text-[var(--admin-text-muted)]"> <IconChevronLeft size={18} /> </button> <p className="font-bold text-sm text-[var(--admin-text-base)]">Sửa liên kết</p> </div> <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar"> <div> <label className={labelCls}>Tên hiển thị</label> <input className={inputCls} value={editingItem.label} onChange={e => setEditingItem({ ...editingItem, label: e.target.value })} /> </div> <div> <label className={labelCls}>Đường dẫn (URL)</label> <input className={inputCls + " dir-ltr text-left"} value={editingItem.url} onChange={e => setEditingItem({ ...editingItem, url: e.target.value })} /> </div> <div> <label className={labelCls}>Vị trí trên thanh menu</label> <select className={inputCls} value={editingItem.group} onChange={e => setEditingItem({ ...editingItem, group: e.target.value as 'primary' | 'secondary' })}> <option value="primary">Bên trái</option> <option value="secondary">Bên phải</option> </select> </div> <div> <label className={labelCls}>Nhãn nhỏ (không bắt buộc)</label> <input className={inputCls} value={editingItem.badge || ''} onChange={e => setEditingItem({ ...editingItem, badge: e.target.value })} placeholder="Ví dụ: Mới" /> </div> <label className="flex items-center gap-3 cursor-pointer"> <input type="checkbox" checked={editingItem.openInNewTab} onChange={e => setEditingItem({ ...editingItem, openInNewTab: e.target.checked })} className="w-4 h-4 rounded text-[var(--admin-primary)]" /> <span className="text-sm text-[var(--admin-text-base)]">Mở trong tab mới</span> </label> </div> <div className="p-4 border-t border-[var(--admin-border-light)] flex gap-2"> <Button variant="secondary" size="sm" onClick={() => setEditingItem(null)} className="flex-1">Hủy</Button> <Button variant="primary" size="sm" isLoading={saving} leftIcon={<IconDeviceFloppy size={15} />} onClick={handleSaveEdit} className="flex-1">Lưu</Button> </div> </div>
 ) : (
 <> <div className="p-4 border-b border-[var(--admin-border-light)] flex items-center justify-between"> <div className="flex items-center gap-2 text-[var(--admin-text-base)]"> <IconSitemap size={20} className="text-[var(--admin-primary)]" /> <h1 className="font-bold">Quản lý menu</h1> </div> <Button variant="primary" size="sm" leftIcon={<IconPlus size={16} />} onClick={() => setShowAddForm(s => !s)}>
 ...
 {/* Add item form */}
 {showAddForm && (
 <div className="p-4 border-b border-[var(--admin-border-light)] bg-[var(--admin-bg-elevated)] space-y-3"> <p className="text-xs font-bold text-[var(--admin-text-base)]">Liên kết mới</p> <input className={inputCls} placeholder="Tên hiển thị" value={newItem.label} onChange={e => setNewItem({ ...newItem, label: e.target.value })} /> <input className={inputCls + " dir-ltr text-left"} placeholder="/shop" value={newItem.url} onChange={e => setNewItem({ ...newItem, url: e.target.value })} /> <select className={inputCls} value={newItem.group} onChange={e => setNewItem({ ...newItem, group: e.target.value as 'primary' | 'secondary' })}> <option value="primary">Bên trái</option> <option value="secondary">Bên phải</option> </select> <div className="flex gap-2"> <Button variant="secondary" size="sm" onClick={() => { setShowAddForm(false); setNewItem(EMPTY_NEW_ITEM); }} className="flex-1">Hủy</Button> <Button variant="primary" size="sm" isLoading={saving} onClick={handleAddItem} className="flex-1">Thêm</Button> </div> </div>
 )}
 ...
 {item.group === 'primary' ? 'Bên trái' : 'Bên phải'}
 ...
 </nav> <div className="text-2xl font-black tracking-tighter text-gray-900 shrink-0">Soft Muse</div> <nav className="flex items-center gap-6">
 ...
 Nội dung trang</div> </div> </CMSPreviewPanel> </div> </FadeIn>
 );
}

function isValidMenuUrl(value: string) {
