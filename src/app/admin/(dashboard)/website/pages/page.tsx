  ...
  return (
    <FadeIn className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 border-b border-[var(--admin-border-light)] custom-scrollbar">
        {[
          { id: 'order_tracking', label: 'تتبع الطلبات' },
          { id: 'general', label: 'نصوص عامة' },
          { id: 'pages', label: 'صفحات' },
          { id: 'emails', label: 'إيميلات' },
        ].map(tab => (
          <button
            key={tab.id}
      ...
      <div className="bg-[var(--admin-bg-surface)] rounded-[var(--admin-radius-xl)] border border-[var(--admin-border-base)] overflow-hidden">
        {loading ? (
          <div className="p-8 text-center text-[var(--admin-text-muted)] animate-pulse">جاري التحميل...</div>
        ) : (
          <StaggerContainer className="divide-y divide-[var(--admin-border-light)]">
            {content.map(block => (
              <StaggerItem key={block.id} className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <h3 className="font-bold text-[var(--admin-text-base)] text-sm">{block.description}</h3>
                  <code className="text-xs text-[var(--admin-text-subtle)] mt-1 inline-block bg-[var(--admin-bg-elevated)] px-2 py-1 rounded">
                    {block.key}
                  </code>
                </div>
                <div className="md:col-span-2 space-y-3 flex flex-col items-end">
                  {block.group === 'pages' ? (
                    <textarea
                  ...
                  {savingId === block.id && <span className="text-xs text-[var(--admin-primary)] font-semibold flex items-center gap-1"><IconDeviceFloppy size={14} /> جاري الحفظ...</span>}
                </div>
              </StaggerItem>
            ))}
            {content.length === 0 && (
