...
export default function SEOManager() {
 ...
 SEOService.getAll()
.then(setAllSEO)
.catch(() => toast.error("tảiCài đặt SEO"))
.finally(() => setLoading(false));
 ...
 toast.success("đãLưu Cài đặt SEO");
 } catch {
 toast.error("Lưu");
 } finally {
 setSaving(false);
 ...
 {/* Meta Title */}
 <div> <label className="block text-sm font-bold text-[var(--admin-text-base)] mb-1">tiêu đề trang (Meta Title)</label> <input
 type="text"
 value={currentSEO.title}
 onChange={e => updateField('title', e.target.value)}
 placeholder=":AURA | Việt Nam"
 ...
 {/* Meta Description */}
 <div> <label className="block text-sm font-bold text-[var(--admin-text-base)] mb-1">mô tả trang (Meta Description)</label> <textarea
 ...
 {/* Keywords */}
 <div> <label className="block text-sm font-bold text-[var(--admin-text-base)] mb-1">Từ khóa (Keywords)</label> <input
 type="text"
 value={currentSEO.keywords}
 onChange={e => updateField('keywords', e.target.value)}
 placeholder="AURA, couture, Thời trang nữ, trang phục cao cấp"
 className="w-full bg-[var(--admin-bg-base)] border border-[var(--admin-border-base)] rounded-md px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--admin-primary)]"
 /> </div>

 {/* OG Image */}
 <div> <label className="block text-sm font-bold text-[var(--admin-text-base)] mb-1"> <IconPhoto size={14} className="inline me-1 mb-0.5" />
 ...
 className="w-full bg-[var(--admin-bg-base)] border border-[var(--admin-border-base)] rounded-md px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--admin-primary)]"
 /> <p className="text-xs text-[var(--admin-text-muted)] mt-1">Kích cỡ : 1200× 630 </p>
 {currentSEO.ogImage && (
 <img
 ...
 {/* Robots + Twitter Card */}
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> <div> <label className="block text-sm font-bold text-[var(--admin-text-base)] mb-1">Robots Meta</label> <select
 value={currentSEO.robots}
 onChange={e => updateField('robots', e.target.value)}
 className="w-full bg-[var(--admin-bg-base)] border border-[var(--admin-border-base)] rounded-md px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--admin-primary)]"
 > <option value="index, follow">index, follow</option> <option value="noindex, follow">noindex, follow</option> <option value="index, nofollow">index, nofollow</option> <option value="noindex, nofollow">noindex, nofollow</option> </select> </div> <div> <label className="block text-sm font-bold text-[var(--admin-text-base)] mb-1">Twitter Card</label> <select
 value={currentSEO.twitterCard}
 onChange={e => updateField('twitterCard', e.target.value as SEOSettings['twitterCard'])}
 className="w-full bg-[var(--admin-bg-base)] border border-[var(--admin-border-base)] rounded-md px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--admin-primary)]"
 > <option value="summary_large_image">summary_large_image</option> <option value="summary">summary</option> </select> </div> </div>
 ...
 /> <span className="text-sm font-bold text-[var(--admin-text-base)]">JSON-LD Schema</span> </label> </div> </div>

 {/* ── Right: Google SERP preview ── */}
 <div className="w-full md:w-[380px] shrink-0 space-y-4"> <div className="bg-white border border-gray-200 rounded-[var(--admin-radius-lg)] p-5 shadow-sm font-sans"> <div className="flex items-center gap-2 mb-4 text-gray-500 text-sm font-medium"> <IconBrandGoogle size={18} />
 Google SERP Preview
 </div> <div className="flex flex-col gap-1 text-left" dir="ltr"> <div className="flex items-center gap-2 text-sm text-gray-800"> <div className="w-7 h-7 bg-gray-100 rounded-full flex items-center justify-center font-bold text-xs shrink-0">A</div> <div> <p className="text-sm leading-tight font-medium">AURA</p> <p className="text-xs text-gray-500 leading-tight">https://aurabrand.com ›.</p> </div> </div> <h3 className="text-[20px] text-[#1a0dab] hover:underline cursor-pointer font-medium leading-snug mt-1 line-clamp-1">
 {currentSEO.title || 'tiêu đề trang'}
 </h3> <p className="text-[14px] text-[#4d5156] line-clamp-2 leading-[1.58]">
 {currentSEO.description || 'mô tả trang trongTìm kiếm.'}
 </p> </div> </div>

 {/* Score badge */}
 <div className="bg-[var(--admin-bg-surface)] border border-[var(--admin-border-base)] rounded-[var(--admin-radius-lg)] p-4 flex items-center gap-3"> <div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${
 ...
 {currentSEO.title && currentSEO.description && currentSEO.ogImage ? '✓' :
 currentSEO.title || currentSEO.description ? '~' : '!'}
 </div> <div> <p className="text-sm font-bold text-[var(--admin-text-base)]">
 {currentSEO.title && currentSEO.description && currentSEO.ogImage
 ? 'Hoàn tất' : currentSEO.title || currentSEO.description
 ? '' : 'trống'}
 </p> <p className="text-xs text-[var(--admin-text-muted)]">
 {!currentSEO.title && 'Địa chỉ.'}
 {!currentSEO.description && 'Mô tả.'}
 {!currentSEO.ogImage && 'ảnhOG.'}
 {currentSEO.title && currentSEO.description && currentSEO.ogImage && 'Tất cả Hoàn tất.'}
 </p> </div> </div> </div> </FadeIn>
 );
}
