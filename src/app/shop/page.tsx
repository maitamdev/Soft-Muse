...
export default function ShopPage() {
  ...
  const categories = ["الكل", "مجموعة المساء الكوتور", "أطقم قطعتين", "بلوزات", "فساتين كاجوال", "قمصان", "بنطلونات"];
  ...
  const filteredProducts = useMemo(() => {
    return mockProducts.filter((product) => {
      ...
      const matchesCategory =
        selectedCategory === "الكل" || product.collection === selectedCategory;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-8 md:gap-12 items-start">
          ...
          {showFilters && (
            <aside className="bg-background-secondary border border-brand-border p-6 flex flex-col gap-6 lg:sticky lg:top-24">
              ...
              {/* Filter 1: Size list */}
              <div>
                <h3 className="font-sans text-xs font-bold text-text-primary border-b border-brand-border pb-2 mb-3">المقاسات المتاحة</h3>
                <div className="flex flex-wrap gap-2">
                  {sizes.map((sz) => {
                    return (
                      <span
                        key={sz}
                        className="text-xs px-3 py-1.5 border border-brand-border text-text-secondary bg-background-primary"
                      >
                        {sz}
                      </span>
                    );
