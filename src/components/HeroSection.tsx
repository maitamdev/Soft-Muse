...
const DEFAULT_SLIDES: HeroSlide[] = [
 ...
 label: "SOFT MUSE OFFICEWEAR",
 title: "Thanh lịch mỗi ngày, tự tin theo cách của bạn",
 engTitle: "MINIMAL LUXURY FOR WORK",
 subtitle: "Áo sơ mi, váy, blazer và set đồ công sở nữ trong mức giá dễ tiếp cận."
 },
 {
 id: 2,
 image: "/images/campaign/campaign_5.png",
 label: "NEW ARRIVALS",
 title: "Những bản phối nhẹ nhàng cho tuần làm việc mới",
 engTitle: "SOFT NEUTRALS",
 subtitle: "Tông kem, trắng, hồng đất và đen cho vẻ ngoài tinh tế, hiện đại."
 },
 {
 id: 3,
 image: "/images/campaign/campaign_6.png",
 label: "BESTSELLER EDIT",
 title: "Các thiết kế được nàng công sở yêu thích",
 engTitle: "WORKDAY ESSENTIALS",
 subtitle: "Phom dáng dễ mặc, chất liệu mềm và đường may chỉn chu cho nhịp sống bận rộn."
 }
];
...
export default function HeroSection() {
 ...
 const [ctaText, setCtaText] = useState(initialSettings?.ctaText || 'Khám phá bộ sưu tập');
 const [ctaLink, setCtaLink] = useState(initialSettings?.ctaLink || '/shop');
 const [secondaryCtaText, setSecondaryCtaText] = useState(initialSettings?.secondaryCtaText || 'Câu chuyện thương hiệu');
 ...
 <motion.div 
 style={{ y: contentY }}
 className="relative z-20 w-full lg:h-full flex flex-col justify-between p-8 sm:p-12 md:p-16 lg:p-24 bg-background-primary text-left order-2 lg:order-1 select-none pointer-events-none lg:pointer-events-auto"
 >
 {/* Subtle Campaign Counter */}
 <div className="hidden lg:flex justify-between items-center text-[10px] uppercase font-sans tracking-[0.2em] text-text-secondary border-b border-brand-border/60 pb-4"> <span>SOFT MUSE / OFFICEWEAR BOUTIQUE</span> <span>0{currentIndex + 1} &mdash; 0{slides.length}</span> </div> <div className="my-auto py-12 flex flex-col items-start text-left w-full max-w-xl self-end"> <AnimatePresence mode="wait"> <motion.div
 key={currentIndex}
 ...
 <div className="flex items-center gap-2"> <span className="font-sans text-[10px] md:text-[11px] text-accent tracking-[0.3em] font-bold uppercase">
 ...
 {/* CTA Actions */}
 <div className="flex flex-row items-center gap-4 w-full mt-8 md:mt-12 pointer-events-auto"> <Link href={ctaLink} className="group flex items-center gap-3 px-8 py-3.5 bg-text-primary text-background-primary font-sans text-[10px] md:text-[11px] uppercase tracking-wider transition-all duration-500 hover:bg-accent hover:text-text-primary"> <span>{ctaText}</span> <ArrowLeft className="w-3.5 h-3.5 transition-transform duration-500 group-hover:-translate-x-1.5" /> </Link> <Link href={secondaryCtaLink} className="px-8 py-3.5 border border-brand-border text-text-primary font-sans text-[10px] md:text-[11px] uppercase tracking-wider transition-all duration-500 hover:border-accent hover:text-accent">
 {secondaryCtaText}
 </Link> </div> </div>
 ...
 <div className="flex gap-4 items-center"> <motion.button
 onClick={prevSlide}
 ...
 aria-label="Thao tác"
 > <ChevronRight className="w-4 h-4" /> </motion.button> <motion.button
 onClick={nextSlide}
 whileHover={{ scale: 1.1 }}
 whileTap={{ scale: 0.92 }}
 transition={{ duration: 0.2, ease: "easeOut" }}
 className="w-8 h-8 rounded-full border border-brand-border/80 flex items-center justify-center text-text-secondary hover:text-accent hover:border-accent transition-colors cursor-pointer"
 aria-label="Thao tác"
 > <ChevronLeft className="w-4 h-4" /> </motion.button> </div>

 {/* Luxury Linear Progress Indicator */}
 <div className="flex items-center gap-4"> <span className="text-[10px] font-sans text-text-secondary font-bold"></span> <div className="w-24 md:w-36 h-[1.5px] bg-brand-border/40 relative overflow-hidden"> <div 
 ...
 <div className="absolute inset-0 bg-gradient-to-t from-text-primary/70 via-text-primary/10 to-transparent pointer-events-none lg:hidden" /> <div className="absolute inset-y-0 right-0 w-16 bg-gradient-to-l from-background-primary/20 to-transparent pointer-events-none hidden lg:block" /> </motion.div> </div> </div>

 {/* Elegant Vertical Scroll Storyteller Indicator */}
 <div className="absolute bottom-6 left-1/2 -translate-x-1/2 lg:left-8 lg:-translate-x-0 z-30 hidden md:flex flex-col items-center gap-3 pointer-events-none"> <span className="text-[9px] font-sans text-text-secondary tracking-[0.35em] uppercase vertical-text">
 SCROLL
 </span> <div className="w-[1px] h-12 bg-brand-border/40 relative overflow-hidden"> <motion.div 
