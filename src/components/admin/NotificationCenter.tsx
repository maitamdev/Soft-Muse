...
const mockNotifications: Notification[] = [
 { id: '1', category: 'Orders', title: 'đơn hàngMới #1024', message: 'đã đơn hàngMới 12,500 đ', isRead: false, time: '5' },
 { id: '2', category: 'Inventory', title: 'Sắp hết hàng', message: 'sản phẩm"" (5)', isRead: false, time: '30' },
 { id: '3', category: 'CRM', title: 'khách hàngMới', message: '"" Mới', isRead: true, time: '' },
 { id: '4', category: 'System', title: '', message: 'đã thành công', isRead: true, time: '5' },
];
...
export function NotificationCenter() {
 ...
 {/* Dropdown */}
 {isOpen && (
 <> <div className="fixed inset-0 z-40" onClick={() => setIsOpen(false)} /> <div className="absolute left-0 top-full mt-2 w-[400px] z-50 rounded-[var(--admin-radius-xl)] bg-[var(--admin-bg-surface)] shadow-[var(--admin-shadow-float)] border border-[var(--admin-border-base)] overflow-hidden">

 {/* Header */}
 <div className="flex items-center justify-between p-4 border-b border-[var(--admin-border-base)] bg-[var(--admin-bg-card)]"> <h3 className="font-bold text-[var(--admin-text-base)]">Thông báo</h3> <div className="flex items-center gap-2"> <button
 onClick={markAllRead}
 className="text-xs font-medium text-[var(--admin-primary)] hover:underline flex items-center gap-1"
 > <IconCircleCheck size={14} />
 Chọn tất cả </button> </div> </div>

 {/* Categories Scroll */}
 <div className="flex overflow-x-auto p-2 gap-1 border-b border-[var(--admin-border-light)] scrollbar-hide"> <button
 ...
 </div> <div className="flex flex-1 flex-col"> <div className="flex items-start justify-between"> <span className={cn(
 "text-sm font-medium",
 !notif.isRead ? "text-[var(--admin-text-base)]" : "text-[var(--admin-text-muted)]"
 )}>
 {notif.title}
 </span> <span className="text-[10px] text-[var(--admin-text-subtle)] whitespace-nowrap mr-2">
 {notif.time}
 </span> </div> <p className={cn(
 ...
 title="AURA"
 > <IconCircleCheck size={14} /> </button>
 )}
 <button
 ...
 title="Lưu trữ/Xóa"
 > <IconX size={14} /> </button> </div> </div>
 ))}
 </div>
 ) : (
 <div className="flex flex-col items-center justify-center p-8 text-[var(--admin-text-subtle)]"> <IconBell size={32} className="mb-2 opacity-20" /> <p className="text-sm">Không có </p> </div>
 )}
 </div>

 {/* Footer */}
 <div className="p-2 border-t border-[var(--admin-border-light)] bg-[var(--admin-bg-base)]"> <button className="w-full text-center text-sm font-medium text-[var(--admin-primary)] hover:underline py-2">
 hiển thị tất cảThông báo
 </button> </div> </div> </>
 )}
 </div>
 );
