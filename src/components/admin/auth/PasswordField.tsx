...
export const PasswordField = React.forwardRef<HTMLInputElement, PasswordFieldProps>(
 ...
 {passwordLength > 0 && (
 <span className="text-[10px] font-mono text-[var(--admin-text-subtle)] bg-[var(--admin-bg-active)] px-1.5 py-0.5 rounded select-none flex items-center gap-0.5"> <span className="tracking-[0.08em]">{"•".repeat(Math.min(passwordLength, 6))}</span>
 {passwordLength > 6 && "+"}
 <span className="ms-1 font-semibold">{passwordLength}ch</span> </span>
 )}
 <button
 type="button"
