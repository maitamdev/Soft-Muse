...
export function ExpenseModal({ isOpen, onClose, onSave, initialData }: ExpenseModalProps) {
 ...
 if (!formData.name.trim()) return setError('Tênchi phí ');
 ...
 isOpen={isOpen} 
 onClose={onClose} 
 title={initialData ? 'Sửa chi phí' : 'Thêm chi phí Mới'} 
 maxWidth="2xl"
 ...
 placeholder=":"
 /> </div> <div className="flex flex-col gap-1"> <label className="text-sm font-medium text-[var(--admin-text-muted)]">Phân loại *</label> <select 
 ...
 > <option value=""></option> <option value=""></option> <option value=""></option> <option value=""></option> <option value=""></option> </select> </div> <div className="flex flex-col gap-1"> <label className="text-sm font-medium text-[var(--admin-text-muted)]">Số tiền *</label> <div className="flex relative"> <input 
 type="number" 
 required
 min="0"
 step="0.01"
 ...
 /> </div> <div className="flex flex-col gap-1"> <label className="text-sm font-medium text-[var(--admin-text-muted)]">Phương thức thanh toán</label> <select 
 value={formData.paymentMethod}
 onChange={e => setFormData({ ...formData, paymentMethod: e.target.value})}
 className="w-full px-3 py-2 border border-[var(--admin-border-base)] rounded-lg bg-[var(--admin-bg-surface)] text-[var(--admin-text-base)] focus:ring-2 focus:ring-[var(--admin-primary)] focus:border-[var(--admin-primary)] outline-none h-12"
 > <option value=""></option> <option value=""></option> <option value=""></option> </select> </div> <div className="col-span-2 flex flex-col gap-1"> <label className="text-sm font-medium text-[var(--admin-text-muted)]">Trạng thái</label> <select 
 value={status}
 onChange={e => setStatus(e.target.value as any)}
 className="w-full px-3 py-2 border border-[var(--admin-border-base)] rounded-lg bg-[var(--admin-bg-surface)] text-[var(--admin-text-base)] focus:ring-2 focus:ring-[var(--admin-primary)] focus:border-[var(--admin-primary)] outline-none h-12"
 > <option value="paid">Đã thanh toán</option> <option value="pending">Tạm khóa</option> <option value="cancelled"></option> </select> </div> <div className="col-span-2"> <TextArea 
 label="Ghi chú"
