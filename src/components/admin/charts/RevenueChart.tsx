...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: any[]; label?: string }) {
  ...
  return (
    <div className="bg-[var(--admin-bg-surface)] border border-[var(--admin-border-strong)] rounded-[var(--admin-radius-lg)] px-4 py-3 shadow-[var(--admin-shadow-lg)] min-w-[160px] animate-in fade-in zoom-in-95 duration-200">
      <p className="text-sm font-semibold text-[var(--admin-text-subtle)] mb-2">{label}</p>
      {payload.map((entry, i) => (
    ...
    <div className="h-full w-full min-h-[300px]">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
          <defs>
            <linearGradient id="adminGradientRevenue" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%"   stopColor="var(--admin-primary)" stopOpacity={0.3} />
              <stop offset="95%" stopColor="var(--admin-primary)" stopOpacity={0}    />
            </linearGradient>
            <linearGradient id="adminGradientStroke" x1="0" y1="0" x2="1" y2="0">
