...
function SkeletonRow({ cols, rowIndex }: { cols: number; rowIndex: number }) {
 ...
 <td key={i} className="px-5 py-4"> <div 
...
export function DataTable<T extends { id?: string | number }>({
 ...
 if (col.type === "price") return <span className="tabular-nums font-semibold">{Number(raw).toLocaleString("en-US")} đ</span>;
 ...
 <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border-b border-[var(--admin-border-light)] bg-[var(--admin-bg-surface)] gap-4"> <div className="flex items-center gap-3">
 ...
 <div className="relative flex-1 sm:flex-none"> <IconSearch size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--admin-text-subtle)]" /> <Input 
 ...
 <Button variant="outline" size="sm" leftIcon={<IconFilter size={16} className="hidden sm:block" />}> <span className="hidden sm:inline">Lọc</span> </Button> <div className="relative" ref={columnManagerRef}> <Button 
 variant="outline" 
 size="sm" 
 ...
 {showColumnManager && (
 <div className="absolute left-0 top-full mt-2 w-48 rounded-[var(--admin-radius-lg)] border border-[var(--admin-border-base)] bg-[var(--admin-bg-surface)] p-2 shadow-lg z-50 animate-in fade-in zoom-in-95"> <div className="text-xs font-semibold text-[var(--admin-text-subtle)] mb-2 px-2">/</div>
 {columns.map((col, idx) => (
 <label key={idx} className="flex items-center gap-2 px-2 py-1.5 hover:bg-[var(--admin-bg-hover)] rounded-[var(--admin-radius-sm)] cursor-pointer"> <input 
 type="checkbox" 
 checked={!col.hidden}
 onChange={() => {
 ...
 /> <span className="text-sm font-medium text-[var(--admin-text-base)]">{col.header}</span> </label>
 ))}
 </div>
 )}
 </div>
 ...
 {onExport && (
 <Button variant="outline" size="sm" leftIcon={<IconDownload size={16} className="hidden sm:block" />} onClick={onExport}> <span className="hidden sm:inline">Xuất</span> </Button>
 )}
 </div> </div>
 )}
 ...
 <tr>
 {selectable && (
 <th className="px-5 py-4 w-12 text-left border-r border-[var(--admin-border-light)]"> <input 
 type="checkbox" 
 ...
 col.align === "end" ? "text-end" : col.align === "center" ? "text-center" : "text-left",
 ...
 {hasActions && (
 <th className="px-5 py-4 text-xs font-semibold text-[var(--admin-text-subtle)] text-end w-[140px] whitespace-nowrap">Thao tác</th>
 )}
 </tr>
 </thead>
 ...
 <tr> <td colSpan={visibleColumns.length + (selectable ? 1 : 0) + (hasActions ? 1 : 0)} className="py-24">
 {emptyState ?? (
 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center gap-4 text-[var(--admin-text-muted)]"> <div className="w-16 h-16 rounded-[var(--admin-radius-xl)] bg-[var(--admin-bg-elevated)] flex items-center justify-center border border-[var(--admin-border-light)] shadow-sm"> <IconSettings2 size={28} stroke={1.5} className="text-[var(--admin-text-subtle)]" /> </div> <div className="text-center"> <p className="text-base font-semibold text-[var(--admin-text-base)]">Không có </p> <p className="text-sm mt-1">Sửa Thêm Mới.</p> </div> </motion.div>
 )}
 </td> </tr>
 ) : (
 ...
 >
 {selectable && (
 <td className="px-5 py-4 w-12 border-r border-[var(--admin-border-light)]" onClick={e => e.stopPropagation()}> <input 
 type="checkbox" 
 checked={isSelected}
 ...
 {hasActions && (
 <td className="px-5 py-4 text-end" onClick={e => e.stopPropagation()}> <div className="flex items-center justify-end gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
 {onEdit && (
 <Button variant="ghost" size="icon-sm" onClick={() => onEdit(item)} title="Sửa" aria-label="Sửa"> <IconEdit size={16} /> </Button>
 )}
 {onDuplicate && (
 <Button variant="ghost" size="icon-sm" onClick={() => onDuplicate(item)} title="AURA" aria-label="Thao tác"> <IconCopy size={16} /> </Button>
 )}
 {onArchive && (
 <Button variant="ghost" size="icon-sm" onClick={() => onArchive(item)} title="Lưu trữ" aria-label="Lưu trữ"> <IconArchive size={16} /> </Button>
 )}
 {onDelete && (
 <Button variant="ghost" size="icon-sm" onClick={() => onDelete(item)} title="Xóa" aria-label="Xóa" className="text-[var(--admin-danger)] hover:bg-[var(--admin-danger-muted)]"> <IconTrash size={16} /> </Button>
 )}
 </div> </td>
 )}
 </motion.tr>
 ...
 <div className="flex items-center justify-between px-5 py-4 border-t border-[var(--admin-border-light)] bg-[var(--admin-bg-surface)]"> <p className="text-xs font-semibold text-[var(--admin-text-muted)]">
 hiển thị{((page - 1) * pageSize) + 1}–{Math.min(page * pageSize, sorted.length)} từ{sorted.length}
 </p> <div className="flex items-center gap-1.5"> <button
 ...
 className="p-1.5 rounded-[var(--admin-radius-md)] hover:bg-[var(--admin-bg-hover)] hover:text-[var(--admin-text-base)] disabled:opacity-30 transition-colors text-[var(--admin-text-muted)]"
 > <IconChevronLeft size={18} /> </button> </div> </div>
 )}
 </div>
 );
}
