    ...
    >
      <div className="space-y-1 min-w-0">
        <div className="flex items-center gap-2.5 flex-wrap">
          {backLink && (
            <Link href={backLink}>
              <Button variant="ghost" size="icon-sm" className="me-1" leftIcon={<IconArrowRight size={18} />} aria-label="Back" />
            </Link>
          )}
...
export function SectionHeader({ title, subtitle, action, className }: SectionHeaderProps) {
  return (
    <div className={cn("flex items-start justify-between gap-3 mb-4", className)}>
      <div>
        <h2 className="text-base font-semibold text-[var(--admin-text-base)]">{title}</h2>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        <h3 className="text-base font-semibold text-[var(--admin-text-base)]">{title}</h3>
