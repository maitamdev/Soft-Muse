...
export function Modal({ isOpen, onClose, title, children, footer, maxWidth, size }: ModalProps) {
 ...
 <AnimatePresence>
 {isOpen && (
 <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6"> <motion.div 
 initial={{ opacity: 0 }}
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 ...
 role="dialog"
 aria-modal="true"
 > <div className="flex items-center justify-between p-6 border-b border-[var(--admin-border-base)] bg-[var(--admin-bg-card)]"> <h2 className="text-xl font-semibold text-[var(--admin-text-base)] tracking-tight">{title}</h2> <button 
 onClick={onClose}
 ...
 > <IconX size={20} stroke={2.5} /> </button> </div> <div className="p-6 overflow-y-auto custom-scrollbar bg-[var(--admin-bg-base)]">
 {children}
 </div>
 {footer && (
