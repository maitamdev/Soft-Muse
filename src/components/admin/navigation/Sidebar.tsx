...
function NavItemComponent({
 ...
 return (
 <Link key={sub.nameKey} href={sub.path} className="block group/sub"> <div className={cn(
...
function SidebarContent({
 ...
 )}> <div className="w-8 h-8 rounded-[var(--admin-radius-md)] bg-[var(--admin-primary)] flex items-center justify-center shrink-0 shadow-lg shadow-[var(--admin-primary)]/30"> <IconSparkles size={18} stroke={2} className="text-white" /> </div> <AnimatePresence initial={false}>
 {!isCollapsed && (
 <motion.div
 initial={{ opacity: 0, width: 0 }}
 animate={{ opacity: 1, width: "auto" }}
 ...
 transition={{ duration: 0.2 }}
 className="overflow-hidden"
 > <p className="text-[var(--admin-text-base)] font-bold text-[15px] tracking-tight whitespace-nowrap leading-tight">Soft Muse Admin</p> <p className="text-[var(--admin-primary)] font-medium text-[10px] whitespace-nowrap">Commerce Studio</p> </motion.div>
 )}
 </AnimatePresence>
 {!isCollapsed && (
 <button
 ...
 > <IconChevronRight size={18} stroke={2} /> </button>
 )}
 </div>

 {isCollapsed && (
 <div className="hidden md:flex justify-center pb-4 pt-2"> <button
 ...
 > <IconChevronLeft size={18} stroke={2} /> </button> </div>
 )}
 ...
 return (
 <div key={groupIdx} className={cn("mb-6", isCollapsed && "mb-4")}> <AnimatePresence initial={false}>
 ...
 <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-[var(--admin-radius-lg)] bg-[var(--admin-bg-base)] border border-[var(--admin-border-base)]"> <img
 src={user.avatarUrl || "https://api.dicebear.com/7.x/notionists/svg?seed=Admin"}
 alt={user.name}
 className="w-7.5 h-7.5 rounded-full bg-[var(--admin-primary-muted)] border border-[var(--admin-border-base)] shrink-0"
 /> <div className="min-w-0 flex-1"> <p className="text-xs font-bold text-[var(--admin-text-base)] truncate leading-tight">{user.name}</p> <p className="text-[10px] text-[var(--admin-text-muted)] truncate mt-0.5">@{user.username}</p> </div> </div>
 )
 )}
 ...
 <div className={cn("rounded-[var(--admin-radius-md)] border px-2 py-1.5", isViewingAs ? "border-[var(--admin-warning)] bg-[var(--admin-warning-muted)]" : "border-[var(--admin-border-base)] bg-[var(--admin-bg-base)]")}> <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[var(--admin-text-subtle)] uppercase tracking-wider mb-1"> <IconShieldCheck size={12} /> </label> <select
 value={viewAsRoleId}
 ...
 > <IconLogout size={18} stroke={1.75} className="shrink-0" /> <AnimatePresence initial={false}>
 {!isCollapsed && (
 <motion.span
 initial={{ opacity: 0, width: 0 }}
 animate={{ opacity: 1, width: "auto" }}
...
export function Sidebar({ isCollapsed, setIsCollapsed, isMobileOpen, setIsMobileOpen }: SidebarProps) {
 ...
 > <div className="h-full w-full bg-[var(--admin-bg-sidebar)] border-l border-[var(--admin-border-base)]"> <SidebarContent
 isCollapsed={false}
 setIsCollapsed={() => {}}
