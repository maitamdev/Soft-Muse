...
export function CMSPreviewPanel({
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--admin-bg-base)] border-l border-[var(--admin-border-base)] overflow-hidden rounded-l-[var(--admin-radius-2xl)]">
      {/* Top Bar */}
      <div className="h-[60px] shrink-0 border-b border-[var(--admin-border-light)] bg-[var(--admin-bg-surface)] px-4 flex items-center justify-between z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold text-[var(--admin-text-base)] ml-2">{title}</span>
          <AnimatePresence>
            {isUpdating && (
              <motion.span
        ...
        <div className="flex items-center bg-[var(--admin-bg-elevated)] p-1 rounded-[var(--admin-radius-lg)] border border-[var(--admin-border-base)]">
          <button
      ...
      {/* Preview Area */}
      <div className="flex-1 overflow-hidden bg-slate-50 dark:bg-zinc-950 flex items-center justify-center p-4 md:p-8">
        <motion.div
          layout
          initial={false}
          animate={{
