...
import {
 IconFileText, IconStack2, IconSlideshow, IconSitemap,
...
export function WebsiteManagerTabs() {
 ...
 return (
 <div className="sticky top-0 z-20 -mx-4 md:-mx-6 px-4 md:px-6 pt-1 pb-2 bg-[var(--admin-bg-base)]/85 backdrop-blur-md border-b border-[var(--admin-border-base)]"> <nav className="flex items-center gap-1 overflow-x-auto custom-scrollbar">
 {TABS.map((tab) => {
 const Icon = tab.icon;
 ...
 > <Icon size={17} stroke={isActive ? 2.4 : 1.8} className="shrink-0" /> <span>{labels[tab.key] ?? tab.key}</span> </Link>
 );
 })}
 </nav> </div>
 );
