...
export function AvatarUpload({ value, name, onChange }: AvatarUploadProps) {
  ...
  const processFile = (file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error('صيغة الملف غير مدعومة. الصيغ المسموحة: JPG, JPEG, PNG, WEBP');
      return;
    }
    ...
    reader.onload = () => {
      ...
      img.onerror = () => {
        toast.error('الملف المحدد تالف أو ليس صورة صالحة.');
      };
      img.src = dataUrl;
    };
  ...
  const handleSaveCrop = async () => {
    ...
    try {
      ...
      toast.error('حدث خطأ أثناء معالجة وقص الصورة.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      {/* Upload/Drop Area */}
      <div className="flex flex-col sm:flex-row items-center gap-6 p-4 rounded-[var(--admin-radius-xl)] bg-[var(--admin-bg-base)] border border-[var(--admin-border-base)]">
        <div 
        ...
        >
          {value ? (
            <>
              <img src={value} alt={name} className="w-full h-full object-cover transition-opacity group-hover:opacity-40" />
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-black/30 text-white">
                <IconCamera size={20} stroke={2} />
                <span className="text-[9px] font-semibold">تغيير الصورة</span>
              </div>
            </>
          ) : (
            <div className="flex flex-col items-center justify-center gap-1 text-[var(--admin-text-subtle)] group-hover:text-[var(--admin-primary)]">
              <span className="text-xl font-bold font-sans tracking-tight">{getInitials()}</span>
              <IconUpload size={16} stroke={2.5} className="mt-0.5" />
            </div>
          )}
          {dragOver && (
        ...
        <div className="flex-1 text-center sm:text-start space-y-2.5">
          <div>
            <h4 className="text-sm font-bold text-[var(--admin-text-base)]">الصورة الشخصية</h4>
            <p className="text-xs text-[var(--admin-text-muted)] mt-1 font-medium">اسحب صورتك وأفلتها هنا أو انقر لاختيارها من جهازك. صيغ JPG, PNG, WEBP بحد أقصى 5 ميجابايت.</p>
          </div>
      ...
      <Modal
        ...
        title="اختر رمزاً جاهزاً"
        size="md"
      >
        <div className="py-4">
          <p className="text-sm text-[var(--admin-text-muted)] mb-5">اختر أحد الرموز الكرتونية الجاهزة لتكون صورتك الشخصية:</p>
          ...
          <div className="flex justify-end mt-6">
            <Button variant="ghost" size="md" onClick={() => setPresetsOpen(false)}>إلغاء</Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal
        ...
        title="تعديل وقص الصورة الشخصية"
        size="lg"
        footer={
          <div className="flex justify-end gap-3">
            <Button variant="ghost" onClick={() => setCropperOpen(false)} disabled={loading}>إلغاء</Button>
            <Button 
              variant="primary" 
      ...
      >
        <div className="py-4 space-y-6">
          <p className="text-sm text-[var(--admin-text-muted)]">اسحب الصورة لتحريكها واستخدم شريط التكبير لضبط حجمها بالشكل المطلوب داخل الدائرة:</p>
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center justify-center">
            ...
            >
              ...
              <div className="absolute inset-0 rounded-full border border-[var(--admin-primary)]/50 pointer-events-none shadow-[0_0_0_9999px_rgba(0,0,0,0.4)]" />
            </div>

            {/* Side tools & Live Preview */}
            <div className="flex-1 w-full space-y-5">
              {/* Live Preview circle */}
              <div className="flex items-center gap-4 p-3.5 rounded-[var(--admin-radius-xl)] bg-[var(--admin-bg-elevated)] border border-[var(--admin-border-base)]">
                <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-[var(--admin-border-strong)] bg-black/40 shrink-0 select-none">
                  <img
                    src={imgSrc}
                    alt="Live preview"
                    style={{
                ...
                <div>
                  <h5 className="text-xs font-bold text-[var(--admin-text-base)]">معاينة مباشرة</h5>
                  <p className="text-[10px] text-[var(--admin-text-muted)] mt-0.5">كيف ستبدو صورتك في القوائم والشريط العلوي.</p>
                </div>
              </div>

              {/* Zoom sliders */}
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs text-[var(--admin-text-muted)] font-medium">
                  <span className="flex items-center gap-1"><IconZoomOut size={14} /> تصغير</span>
                  <span>{(zoom * 100).toFixed(0)}%</span>
                  <span className="flex items-center gap-1"><IconZoomIn size={14} /> تكبير</span>
                </div>
                <input 
                  type="range"
                  min="1"
              ...
              {/* Adjustments row */}
              <div className="flex flex-wrap gap-2 pt-2 border-t border-[var(--admin-border-light)]">
                <Button
                  type="button"
                  variant="secondary"
                  size="sm"
