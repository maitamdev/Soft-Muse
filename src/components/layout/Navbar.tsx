...
export default function Navbar() {
 ...
 const [storeAddress, setStoreAddress] = useState("TP. Hồ Chí Minh, Việt Nam");
 const [storeTitle, setStoreTitle] = useState("Soft Muse");
 ...
 className={`group font-sans text-[14px] lg:text-[15px] font-medium tracking-[0.2px] whitespace-nowrap transition-all duration-300 relative py-2 flex items-center justify-center ${
 ...
 <div className="shrink-0 flex justify-center items-center md:px-5 lg:px-7 xl:px-8"> <Link href="/" className="flex justify-center items-center py-2 select-none" aria-label="Soft Muse Trang chủ"> <Logo size="md" variant="black" animated={true} /> </Link> </div>

 {/* RIGHT: Desktop links + Actions */}
 <div className="flex-1 flex items-center justify-end gap-4 md:gap-6">
 {/* Desktop Right Links */}
 <nav className="hidden md:flex items-center gap-6 lg:gap-8 me-3 lg:me-6">
 {rightLinks.map((link) => {
 const isActive = pathname === link.path;
 return (
 <Link
 ...
 <div className="flex justify-between items-center px-6 py-8 border-b border-brand-border/30"> <Link href="/" onClick={() => setMobileMenuOpen(false)} className="flex items-center"> <Logo size="md" variant="black" animated={false} /> </Link> <button
 ref={mobileMenuCloseRef}
 onClick={() => setMobileMenuOpen(false)}
 className="p-2 -me-2 text-text-primary hover:text-accent transition-colors"
 aria-label="Đóng"
 > <X className="w-7 h-7 stroke-[0.75]" /> </button> </div>

 {/* Navigation Links with Stagger */}
 <motion.div 
 className="flex flex-col gap-8 px-8 mt-12 text-left overflow-y-auto"
 initial="hidden"
 animate="visible"
 variants={{
 hidden: { opacity: 0 },
 ...
 {/* Brand Info */}
 <div className="flex flex-col gap-2 text-[11px] font-sans text-text-secondary font-light"> <p className="font-medium text-text-primary">{storeTitle}</p> <p>{storeAddress}</p> </div> </motion.div> </motion.div> </>
 )}
 </AnimatePresence>
 ...
 {/* Input Bar */}
 <div className="flex items-center gap-4 border-b border-brand-border pb-2"> <Search className="w-5 h-5 text-accent shrink-0" /> <input
 type="text"
 value={searchQuery}
 ...
 <div className="grid grid-cols-1 md:grid-cols-[250px_1fr] gap-8 mt-2 text-left" dir="ltr">
 ...
 onClick={() => handleSuggestionClick(term)}
 className="text-xs text-text-secondary hover:text-text-primary transition-colors text-left py-1 flex-grow"
 >
 {term}
 </button> </li>
 ))}
 ...
 > <div className="relative w-12 h-16 shrink-0 bg-background-secondary border border-brand-border"> <Image src={primaryImage(product)} alt={product.name} fill sizes="48px" className="object-cover" /> </div> <div className="flex flex-col justify-center min-w-0"> <h5 className="font-sans text-xs font-medium text-text-primary group-hover:text-accent transition-colors truncate max-w-[160px]">
 {product.name}
 </h5> <span className="font-display text-xs text-accent font-semibold mt-1">
 ...
 > <div className="relative w-20 h-24 shrink-0 bg-background-secondary border border-brand-border group-hover:border-accent transition-colors"> <Image src={item.image} alt={item.title} fill sizes="80px" className="object-cover" /> </div> <div className="flex-grow flex flex-col justify-between"> <div> <span className="text-[10px] text-accent font-semibold uppercase mb-1 block">
 ...
 > <X className="w-4 h-4 stroke-[1.5]" /> </button> </div>
 ))
 )}
 </div>
 ...
 <nav className="fixed bottom-0 left-0 right-0 z-40 h-16 bg-background-secondary border-t border-brand-border flex items-center justify-around md:hidden px-2 shadow-[0_-4px_12px_rgba(0,0,0,0.03)] pb-safe"> <Link
 href="/"
 ...
 > <PiHouseThin className="w-6 h-6" /> <span className="text-[9px] font-sans">Trang chủ</span> </Link> <Link
 href="/shop"
 className={`flex flex-col items-center justify-center gap-1 w-12 text-center transition-colors ${
 pathname.startsWith("/shop") ? "text-accent font-medium" : "text-text-secondary hover:text-accent"
 }`}
 > <PiCompassThin className="w-6 h-6" /> <span className="text-[9px] font-sans">Cửa hàng</span> </Link> <button
 onClick={() => {
 ...
 > <PiMagnifyingGlassThin className="w-6 h-6" /> <span className="text-[9px] font-sans">Tìm kiếm</span> </button> <Link
 href="/wishlist"
