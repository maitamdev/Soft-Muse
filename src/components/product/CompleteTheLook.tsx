...
export default function CompleteTheLook({ currentProduct }: CompleteTheLookProps) {
 ...
 if (col.includes("") || col.includes("Váy") || col.includes("")) {
 return ["", "", ""];
 }
 ...
 if (col.includes("") || col.includes("") || col.includes("mùa đông")) {
 return ["", ""];
 }
 ...
 return (
 <section className="w-full bg-background-primary border-t border-brand-border py-16 md:py-24 overflow-hidden"> <div className="max-w-[1280px] mx-auto px-6 md:px-12"> <motion.div 
 initial={{ opacity: 0, y: 20 }}
 whileInView={{ opacity: 1, y: 0 }}
 viewport={{ once: true }}
 transition={{ duration: 0.6 }}
 className="text-center mb-12"
 > <span className="font-sans text-xs text-accent font-bold uppercase block mb-2"> </span> <h2 className="font-serif text-3xl md:text-4xl font-light text-text-primary"> </h2> </motion.div>
 ...
 <div key={product.id} className="min-w-[260px] md:min-w-0 flex-shrink-0"> <ProductCard
 id={product.id}
 title={product.name}
 price={product.price}
