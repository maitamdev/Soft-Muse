...
export function AnimatedStars({ rating, max = 5, size = 16, interactive = false, onChange }: AnimatedStarsProps) {
 ...
 return (
 <div className="flex items-center gap-0.5" aria-label={`${rating} từ${max} `}>
...
export function FeatureIcon({ icon: Icon, label, description }: FeatureIconProps) {
 ...
 > <Icon className="w-6 h-6 stroke-[1.25] text-[#C5A880]" aria-hidden="true" /> </motion.div> <p className="font-sans text-xs font-semibold text-text-primary">{label}</p>
 {description && (
