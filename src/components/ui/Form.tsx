...
export function LuxuryInput({ label, error, className = "", id, ...props }: InputProps) {
 ...
 { ...props}
 /> <span className="pointer-events-none absolute bottom-0 start-0 h-[1.5px] w-0 bg-accent transition-all duration-300 ease-out peer-focus:w-full" /> </div>
...
export function LuxurySelect({ label, options, error, className = "", id, ...props }: SelectProps) {
 ...
 </select> <span className="pointer-events-none absolute bottom-0 start-0 h-[1.5px] w-0 bg-accent transition-all duration-300 ease-out peer-focus:w-full" /> <div className="absolute start-2 top-1/2 -translate-y-1/2 pointer-events-none text-accent">
 {/* Custom SVG arrow down */}
 <svg xmlns="http://www.w3.org/2000/svg" width="10" height="6" viewBox="0 0 10 6" fill="none"> <path d="M1 1L5 5L9 1" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/> </svg> </div> </div>
 {error && <span className="text-[10px] text-red-500 font-light mt-1">{error}</span>}
 </div>
 );
}
