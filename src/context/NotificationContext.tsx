...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
 ...
 {/* Toast Render Area */}
 <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[9999] w-full max-w-[400px] px-6 pointer-events-none"> <AnimatePresence mode="wait">
 {notification && (
 <motion.div
 key={notification.id}
 ...
 className={`pointer-events-auto w-full bg-background-secondary border border-brand-border border-r-4 ${accentBorderClass[notification.type]} p-4 shadow-sm flex items-start gap-3 text-left`}
 dir="ltr"
 >
 {/* Accent Icon */}
 ...
 {/* Message Content */}
 <div className="flex-grow"> <p className="font-sans text-xs font-medium text-text-primary leading-normal">
 ...
 aria-label="Đóng"
 > <IconX className="w-3.5 h-3.5 stroke-[1.5]" /> </button> </motion.div>
 )}
 </AnimatePresence> </div> </NotificationContext.Provider>
 );
}
