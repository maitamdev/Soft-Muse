...
export class MockCouponRepository
 ...
 if (!coupon) return { valid: false, coupon: null, reason: 'không' };
 if (coupon.status !== 'active') return { valid: false, coupon: null, reason: 'Không hoạt động' };
 if (coupon.expiryDate && coupon.expiryDate < new Date().toISOString().split('T')[0]) return { valid: false, coupon: null, reason: '' };
 if (coupon.usageLimit !== null && coupon.usageCount >= coupon.usageLimit) return { valid: false, coupon: null, reason: 'đã' };
 if (orderTotal !== undefined && orderTotal < coupon.minimumPurchase) return { valid: false, coupon: null, reason: `${coupon.minimumPurchase} đ` };
