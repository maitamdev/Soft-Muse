...
export class MockGiftCardRepository
  ...
  async validate(code: string) {
    ...
    if (!gc) return { valid: false, balance: 0, reason: 'بطاقة الهدايا غير موجودة' };
    if (gc.status !== 'active') return { valid: false, balance: 0, reason: 'بطاقة الهدايا غير نشطة' };
    if (gc.expiresAt && gc.expiresAt < new Date().toISOString()) return { valid: false, balance: 0, reason: 'انتهت صلاحية بطاقة الهدايا' };
    if (gc.currentBalance <= 0) return { valid: false, balance: 0, reason: 'رصيد بطاقة الهدايا صفر' };
